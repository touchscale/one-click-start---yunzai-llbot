...
BOOTSTRAP_RESOURCES = {
    'bootstrap.bundle.min.js': {
        'url': 'https://cdn.bootcdn.net/ajax/libs/bootstrap/5.3.0/js/bootstrap.bundle.min.js',
        'local_path': 'webui/static/vendor/bootstrap.bundle.min.js',
        'description': 'Bootstrap Bundle JS (包含 Popper)'
    },
    'bootstrap.min.css': {
        'url': 'https://cdn.bootcdn.net/ajax/libs/bootstrap/5.3.0/css/bootstrap.min.css',
...
def download_file(url, local_path):
    ...
    try:
        ...
        # 下载文件
        logger.info(f"开始下载: {url}", extra={'event_type': EventType.INFO, 'url': url})
...
def check_and_update_resources():
    ...
    for name, resource in BOOTSTRAP_RESOURCES.items():
        ...
        logger.info(f"远程文件哈希: {remote_hash[:16]}...", extra={
            'event_type': EventType.INFO,
            'resource': name,
            'hash': remote_hash[:16]
        })
        ...
        if local_hash == remote_hash:
            ...
            if download_file(url, local_path):
                updated_count += 1
                logger.info(f"{description} 更新成功", extra={
                    'event_type': EventType.INFO,
                    'resource': name,
                    'status': 'updated'
                })
...
def force_update_resources():
    ...
    for name, resource in BOOTSTRAP_RESOURCES.items():
        ...
        if download_file(url, local_path):
            updated_count += 1
            logger.info(f"{description} 强制更新成功", extra={
                'event_type': EventType.INFO,
                'resource': name,
